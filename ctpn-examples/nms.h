...
vector<int> nms_proposals(vector< vector<float> >& rois, vector<float>& scores, float thresh)
{
	vector<int> keep;
...
vector<int> nms_tlines(vector< vector<float> >& tlines, float thresh)
{
	vector<int> keep;
	vector<bool> suppressed;
	suppressed.resize(tlines.size());
	...
	for(size_t i=0; i<tlines.size(); i++)
	{
		...
		float y2 = tlines[i][3];
		float area = (x2 - x1 + 1) * (y2 - y1 + 1);
		for(size_t j=i+1; j<tlines.size(); j++)
		{
			if(suppressed[j])
				continue;
			float xx1 = max(tlines[j][0],x1);
			float yy1 = max(tlines[j][1],y1);
			float xx2 = min(tlines[j][2],x2);
			float yy2 = min(tlines[j][3],y2);
			float areaj = (xx2 - xx1 + 1) * (yy2 - yy1 + 1);
			float w = xx2-xx1+1;
			w = w>0?w:0;
			float h = yy2-yy1+1;
